...
function updateTotals(){document.getElementById('bulk-product-count').textContent=data.items.length;document.getElementById('bulk-case-total').textContent=number(data.items.reduce((sum,item)=>sum+item.stores.reduce((total,store)=>total+Number(store.cases||0),0),0));}
function buildGrid(){const columns=[{field:'product_number',headerName:'Product #',pinned:'left',width:110,filter:true,comparator:gridNaturalCompare},{field:'product_name',headerName:'Product',pinned:'left',width:230,filter:ProductNameCategoryFilter,comparator:gridNaturalCompare},{colId:'total_cases',headerName:'Total',pinned:'left',width:82,valueGetter:p=>p.data.stores.reduce((sum,row)=>sum+Number(row.cases||0),0),valueFormatter:p=>number(p.value),type:'numericColumn',comparator:gridNumberCompare},...data.stores.map((store,index)=>({colId:`store_${store.id}`,headerName:store.number,width:126,minWidth:116,headerClass:`${['store-blue','store-green','store-peach','store-lilac','store-mint'][index%5]}-header`,cellClass:`${['store-blue','store-green','store-peach','store-lilac','store-mint'][index%5]}-cell`,valueGetter:p=>p.data.stores.find(row=>row.store_id===store.id),cellRenderer:caseRenderer,comparator:(a,b)=>gridNumberCompare(a?.cases,b?.cases)})),{colId:'remove',headerName:'',pinned:'right',width:42,minWidth:42,maxWidth:42,sortable:false,filter:false,cellRenderer:removeRenderer}];if(!gridApi){gridApi=agGrid.createGrid(document.getElementById('bulk-order-grid'),{columnDefs:columns,rowData:data.items,defaultColDef:{sortable:true,resizable:true},rowHeight:76,getRowId:p=>String(p.data.id),onColumnVisible:()=>{renderBulkColumnPanels();saveBulkLayout();},onColumnPinned:()=>{renderBulkColumnPanels();saveBulkLayout();},onColumnMoved:saveBulkLayout,onColumnResized:event=>{if(event.finished)saveBulkLayout();},onSortChanged:saveBulkLayout,onFilterChanged:saveBulkLayout});restoreBulkLayout();}else{gridApi.setGridOption('columnDefs',columns);gridApi.setGridOption('rowData',data.items);}renderBulkColumnPanels();}
async function loadBulkOrder(){data=await apiFetch(`/api/bulk-orders/${window.BULK_ORDER_ID}/`);buildGrid();updateTotals();document.getElementById('bulk-order-notes').value=data.bulk_order.notes||'';}
...
function restoreBulkLayout(){try{const saved=JSON.parse(localStorage.getItem(BULK_LAYOUT_KEY));if(saved?.columns)gridApi.applyColumnState({state:saved.columns,applyOrder:true});if(saved?.filters)gridApi.setFilterModel(saved.filters);}catch(_){}}
function renderBulkColumnPanels(){if(!gridApi)return;const columns=gridApi.getColumns().filter(column=>column.getColId()!=='remove');document.getElementById('bulk-column-options').innerHTML=columns.map(column=>`<label><input type="checkbox" data-column="${column.getColId()}" ${column.isVisible()?'checked':''}><span>${escapeHtml(column.getColDef().headerName||column.getColId())}</span></label>`).join('');document.getElementById('bulk-freeze-options').innerHTML=columns.map(column=>`<label><input type="checkbox" data-freeze="${column.getColId()}" ${column.getPinned()==='left'?'checked':''}><span>${escapeHtml(column.getColDef().headerName||column.getColId())}</span></label>`).join('');}
function toggleBulkPanel(panelId,button){const panel=document.getElementById(panelId),other=panelId==='bulk-columns-panel'?document.getElementById('bulk-freeze-panel'):document.getElementById('bulk-columns-panel');other.hidden=true;panel.hidden=!panel.hidden;if(!panel.hidden){const rect=button.getBoundingClientRect();panel.style.left=`${Math.max(12,Math.min(rect.left,innerWidth-292))}px`;panel.style.top=`${rect.bottom+7}px`;renderBulkColumnPanels();}}
document.getElementById('bulk-columns-button').addEventListener('click',event=>toggleBulkPanel('bulk-columns-panel',event.currentTarget));document.getElementById('bulk-freeze-button').addEventListener('click',event=>toggleBulkPanel('bulk-freeze-panel',event.currentTarget));document.querySelector('#bulk-columns-panel .icon-button').onclick=()=>document.getElementById('bulk-columns-panel').hidden=true;document.querySelector('#bulk-freeze-panel .icon-button').onclick=()=>document.getElementById('bulk-freeze-panel').hidden=true;
document.getElementById('bulk-column-options').addEventListener('change',event=>{if(event.target.dataset.column)gridApi.setColumnsVisible([event.target.dataset.column],event.target.checked);});document.getElementById('bulk-freeze-options').addEventListener('change',event=>{if(event.target.dataset.freeze)gridApi.applyColumnState({state:[{colId:event.target.dataset.freeze,pinned:event.target.checked?'left':null}]});});document.getElementById('bulk-reset-button').addEventListener('click',()=>{localStorage.removeItem(BULK_LAYOUT_KEY);gridApi.setFilterModel(null);gridApi.resetColumnState();renderBulkColumnPanels();showToast('Grid layout reset');});
const noteDialog=document.getElementById('bulk-note-dialog'),noteInput=document.getElementById('bulk-order-notes'),noteStatus=document.getElementById('bulk-note-status');document.getElementById('bulk-note-button').onclick=()=>{noteDialog.showModal();noteInput.focus();};document.getElementById('bulk-note-close').onclick=()=>noteDialog.close();document.getElementById('bulk-note-done').onclick=()=>noteDialog.close();noteInput.addEventListener('input',()=>{clearTimeout(notesTimer);noteStatus.textContent='Saving…';notesTimer=setTimeout(async()=>{try{await apiFetch(`/api/bulk-orders/${window.BULK_ORDER_ID}/`,{method:'PATCH',body:JSON.stringify({notes:noteInput.value})});noteStatus.textContent='Saved automatically';}catch(error){noteStatus.textContent='Could not save';showToast(error.message,true);}},450);});
