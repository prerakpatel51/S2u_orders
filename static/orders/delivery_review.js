...
async function loadExports() {
  ...
  try {
    ...
    target.innerHTML = data.exports.length ? data.exports.map(item => `<div class="backup-row recovery-export-row"><span class="backup-icon ${item.status}"><i data-lucide="${item.status === 'complete' ? 'file-archive' : item.status === 'failed' ? 'circle-alert' : item.status === 'expired' ? 'clock' : 'loader-circle'}"></i></span><div><strong>${escape(exportDescription(item))}</strong><small>${formatDate(item.created_at)} · ${escape(item.created_by)}${item.status === 'complete' ? ` · ${item.delivery_count} deliveries · ${item.file_count} files · ${formatBytes(item.size_bytes)}` : ''}</small>${item.error ? `<p>${escape(item.error)}</p>` : ''}${item.expires_at ? `<small>Available until ${formatDate(item.expires_at)}</small>` : ''}</div>${item.download_url ? `<a class="primary-button" href="${item.download_url}"><i data-lucide="download"></i>Download ZIP</a>` : `<span class="delivery-status ${item.status === 'failed' ? 'issue' : item.status === 'complete' ? 'verified' : 'pending'}">${escape(item.status_label)}</span>`}</div>`).join('') : '<p class="backup-empty">No recovery ZIP has been built yet.</p>';
    lucide.createIcons();
